...
class Import_MM extends Import {
...
//		print "Yield : " . $this->currentRecipe->serving_size . "<br />\n";
     ...
     *        possible sollution: use hardcoded string indexes (field lengths).
     */
...
//	    print "readIngredientLine: Ingredient: $name <br />\n";
	...
	foreach ($list as $item) {
		...
		$this->currentRecipe->base = $this->bases[$key];
	    } else if ($key = $this->containsValue($this->courses, $item)) {
		$this->currentRecipe->course = $this->courses[$key];
	    } else if ($key = $this->containsValue($this->ethnicity, $item)) {
		$this->currentRecipe->ethnic = $this->ethnicity[$key];
	    }
	}
    } // addCategories
	...
	if ($unit == 'x') {
		...
		$this->currentIngredient->unit = $g_rb_unitmap[($this->MMUnits[$unit])];
