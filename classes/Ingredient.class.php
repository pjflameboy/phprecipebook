...
class Ingredient {
	...
	*/
	function setIngredient($id=0,$core_id='NULL',$name='',$description='',$unit='NULL',$location='NULL',$solid="TRUE", $user='NULL') {
		global $DB_LINK;
		$this->id = $id;
	...
	*/
	function update() {
		...
		$sql = "
			...
			SET ingredient_core='".$DB_LINK->addq($this->core_id, get_magic_quotes_gpc())."',
				ingredient_name='".$DB_LINK->addq($this->name, get_magic_quotes_gpc())."',
	...
	*/
	function loadIngredient() {
		...
		if ($this->name==NULL) {
			$sql = "SELECT $db_table_ingredients.*, unit_desc, location_desc, unit_system, core.description FROM $db_table_ingredients " . 
				"LEFT JOIN $db_table_units ON unit_id = ingredient_unit " .
				"LEFT JOIN $db_table_locations ON location_id = ingredient_location " .
				"LEFT JOIN $db_table_core_ingredients core ON core.id = ingredient_core " . 
				"WHERE ingredient_id=" . $DB_LINK->addq($this->id, get_magic_quotes_gpc());
