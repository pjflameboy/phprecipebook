...
class MealPlan {
	...
	*/
	function MealPlan($date) {
		global $SMObj, $LangUI, $DB_LINK, $db_table_settings;
		...
		$this->startWeekDay = isset($result->fields['setting_value']) ? $result->fields['setting_value'] : 0;  // Default to Sunday
	...
	*/
	function load($start,$end) {
		...
		$sql = "SELECT mplan_date,mplan_meal,mplan_servings,mplan_recipe,recipe_name FROM recipe_mealplans
				...
				WHERE mplan_user = '" . $SMObj->getUserID() . "' AND mplan_date BETWEEN '".$DB_LINK->addq($start, get_magic_quotes_gpc())."' AND '".$DB_LINK->addq($end, get_magic_quotes_gpc())."'";
	...
	*/
	function insert($date, $meal, $recipe, $servings, $user) {
        ...
        $user = $DB_LINK->addq($user, get_magic_quotes_gpc());

		$sql = "INSERT INTO $db_table_mealplans (mplan_date, mplan_meal, mplan_recipe, mplan_servings, mplan_user)
					VALUES ('$date', $meal, $recipe, $servings, '$user')";
		$rc = $DB_LINK->Execute($sql);
