...
class Recipe {
	...
	*/
	function updatePicture($resize='no') {
		global $DB_LINK, $db_table_recipes, $g_rb_database_type, $LangUI, $g_rb_image_resize_width;
		if (is_uploaded_file($this->picture['tmp_name'])) 
		{
			
			if ($resize == 'yes' && $g_rb_image_resize_width != 0 && function_exists('imagesx'))
			{
				$tempImageName = $this->picture['tmp_name'] . "_resized";
				// try to resize the image
				$image = new SimpleImage();
				$image->load($this->picture['tmp_name']);
				$image->resizeToWidth($g_rb_image_resize_width);
				$image->save($tempImageName);
				rename($tempImageName, $this->picture['tmp_name']);
			}
