...
class ShoppingList {
	...
	*/
	...
	/**
	 * Saves a new Shopping list as the CURRENT (reserved) list.  There will
	 * only be one saved current at a time.
	 * @return unknown_type
	 */
	function saveAsCurrentList()
	{
		global $DB_LINK, $db_table_list_names, $SMObj;
		$sql = "DELETE FROM $db_table_list_names WHERE list_name='RB_CURRENT' AND list_user='" . $SMObj->getUserID() . "'";
		$rc = $DB_LINK->Execute($sql);
		DBUtils::checkResult($rc, NULL, NULL, $sql);
		
		$this->saveNewList("RB_CURRENT");
	}
	
	function loadCurrentListId()
	{
		global $DB_LINK, $db_table_list_names,$SMObj;
		$sql = "SELECT list_id FROM $db_table_list_names WHERE list_name='RB_CURRENT' AND list_user='" . $SMObj->getUserID() . "'";
		$rc = $DB_LINK->Execute($sql);
		if (isset($rc->fields['list_id']))
		{
			$this->id = $rc->fields['list_id'];
		}
	}
	...
	*/
	function loadItems($clear) {
		...
		// Add the ingredients
		$sql = "SELECT list_ing_ingredient,list_ing_unit,list_ing_qualifier,list_ing_quantity FROM $db_table_list_ingredients WHERE list_ing_id=" . 
			$DB_LINK->addq($this->id, get_magic_quotes_gpc()) . " ORDER BY list_ing_order";
		$rc = $DB_LINK->Execute($sql);
		DBUtils::checkResult($rc, NULL, NULL, $sql);
		while (!$rc->EOF) {
