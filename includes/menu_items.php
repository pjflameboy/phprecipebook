...
// Administration
if ($SMObj->getUserLoginID()!="") {
	$menu_items['admin'][0] = array( $LangUI->_('Settings'), "index.php?m=admin");
	$menu_items['admin'][1] = array( $LangUI->_('Store Layouts'), "index.php?m=admin&amp;a=stores");
	$menu_items['admin'][2] = array( $LangUI->_('Meal Planner'), "index.php?m=admin&amp;a=mealplan");
	// Administrator only operations
	if ($SMObj->checkAccessLevel("ADMINISTRATOR")) {
		$menu_items['admin'][3] = array( $LangUI->_('Users'), "index.php?m=admin&amp;a=user_admin");
		$menu_items['admin'][4] = array( $LangUI->_('Customize'), "index.php?m=admin&amp;a=customize");
		$menu_items['admin'][5] = array( $LangUI->_('Translation'), "index.php?m=admin&amp;a=translation");
	}
}
...
function printMenu($menu_items) {
	...
	echo '<div class="menuLevelOne">';
	foreach ($menu_items as $key=>$item) {
		echo '<a href="' . $item[0][1] . '" class="menuLink' . (($m == $key) ? ' selectMenuItem' : '') . '">' . $item[0][0] . '</a>  ';
	}
	echo '</div>';
}
...
function printSubMenu($menu_items) {
	...
	<?php if (count($submenu) > 0) { ?>
		...
		<a href="#menuActionsLink" 
		class="fg-button fg-button-icon-right ui-widget ui-state-default ui-corner-all" id="actionMenu" />
		<span class="ui-icon ui-icon-triangle-1-s"></span>Available Actions</a>
