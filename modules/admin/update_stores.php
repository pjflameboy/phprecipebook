...
if ($mode == "add") {
	...
	$store_owner = $SMObj->getUserID();
	$sql = "INSERT INTO $db_table_stores (store_name, store_layout, store_user) VALUES ('".$DB_LINK->addq($store_name, get_magic_quotes_gpc())."', '', '".$DB_LINK->addq($store_owner, get_magic_quotes_gpc())."')";
	...
	while (isset($_POST[$this_store_name])) {
		...
		if (isset($_POST[$store_delete])) {
			...
			$sql = "DELETE FROM $db_table_stores WHERE store_id='" . $DB_LINK->addq($_POST[$this_store_id], get_magic_quotes_gpc()) . "' AND store_user='" . $SMObj->getUserLoginID() . "'";
			$rc = $DB_LINK->Execute($sql);
			DBUtils::checkResult($rc, NULL, NULL, $sql);
		} else {
			...
			$sql = "UPDATE $db_table_stores SET
					...
					"' WHERE store_id='" . $DB_LINK->addq($_POST[$this_store_id], get_magic_quotes_gpc()) . "' AND store_user='" . $SMObj->getUserID() . "'";
			$rc = $DB_LINK->Execute($sql);
			DBUtils::checkResult($rc, NULL, NULL, $sql);
		}
