...
$store_id = (isset($_REQUEST['store_id']) && isValidID($_REQUEST['store_id'])) ? stripslashes( $_REQUEST['store_id'] ) : 0; // the selected store layout
...
$sql = "SELECT store_id, store_name FROM $db_table_stores WHERE store_user = '" . $SMObj->getUserID() . "'";
...
if ($store_id > 0)
{
	...
	$sql = "SELECT store_layout FROM $db_table_stores WHERE store_user = '" . $SMObj->getUserID() . "' AND store_id = " . $DB_LINK->addq($store_id, get_magic_quotes_gpc());
	$rc = $DB_LINK->Execute($sql);
	DBUtils::checkResult($rc, NULL, NULL, $sql);
...
<tr valign="top">
	<td colspan="2">
		...
		<input type="button" value="<?php echo $LangUI->_('Refresh'); ?>" class="button" onclick="submitIt('refresh')">
	</td>
</tr>
<tr>
	<th width="5%"><?php echo $LangUI->_('Remove');?></th>
...
<i><?php echo $LangUI->_('Select the ingredients you wish to remove, then:');?></i>
<input type="button" value="<?php echo $LangUI->_('Print Final List'); ?>" class="button" onclick="submitIt('print')">
<?php if ($SMObj->getUserLoginID() != "") { ?>
<input type="button" value="<?php echo $LangUI->_('Save As Current List');?>" class="button" onclick="submitIt('save')"/>
<?php }?>

</form>
<?php } else if ($mode=='print_confirm') {  ?>
