...
function createEditLink($date, $viewType)
{
	global $LangUI;
	return '<a href="#" onClick="editDay(\'' . $date . "', '" . $viewType . "'); return false;\">[" . $LangUI->_('Edit') . ']<a/>';
}
?>
<!----------------------- Display Header --------------------------------------->
<script type="text/javascript">
$(document).ready(function() {
		$("#editDayDialog").dialog({
			autoOpen: false, 
			modal: true,
			width: 800,
...
function editDay(loadDate, viewType)
{
	$.get('index.php?m=meals&a=edit&print=yes&format=no&view=' + viewType + '&date=' + loadDate, function(data) {
		$('#editDayDialog').html(data).dialog('open').dialog({title: "Edit Meal Plan For " + loadDate });
	});
}
</script>
...
//-------------------------------------------------------------------------------
	...
	while ($month == $MPObj->currentMonth) {
		...
		foreach ($weekdays as $d) {
			...
			echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . createEditLink($d[1] . '-' . $d[0] . '-' . $d[2],"monthly");
