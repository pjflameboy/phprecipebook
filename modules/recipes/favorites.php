...
if ($mode=="add") {
	$sql = "SELECT favorite_recipe FROM $db_table_favorites WHERE favorite_user='" . $SMObj->getUserID() . "' AND favorite_recipe=" . $DB_LINK->addq($recipe_id, get_magic_quotes_gpc());
	$rc = $DB_LINK->Execute( $sql );
	DBUtils::checkResult($rc,NULL,NULL,$sql);
	if ($rc->RecordCount()>0) {
		echo "<font color=red>" . $LangUI->_('Favorite is already in list!') . "</font>";
	} else {
		$sql = "INSERT INTO $db_table_favorites (favorite_user, favorite_recipe) VALUES ('" . $SMObj->getUserID() . "',".$DB_LINK->addq($recipe_id, get_magic_quotes_gpc()).")";
		$rc = $DB_LINK->Execute( $sql );
		DBUtils::checkResult($rc,$LangUI->_('Favorite Successfully Added'),$LangUI->_('Failed to add Favorite'),$sql);
	}
} else if ($mode=="delete") {
	$sql = "DELETE FROM $db_table_favorites WHERE favorite_user='" . $SMObj->getUserID() . "' AND favorite_recipe=" . $DB_LINK->addq($recipe_id, get_magic_quotes_gpc());
	$rc = $DB_LINK->Execute( $sql );
...
$sql = "SELECT recipe_name,favorite_recipe FROM $db_table_favorites
		...
		WHERE favorite_user='" . $SMObj->getUserID() . "'";
$rc = $DB_LINK->Execute( $sql );
DBUtils::checkResult($rc,NULL,NULL,$sql);
