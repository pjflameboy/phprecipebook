...
$sql = "SELECT DISTINCT LOWER(SUBSTRING(recipe_name, 1, 1)) AS A FROM $db_table_recipes WHERE"; 
if ($SMObj->isUserLoggedIn()) {
	$sql .= " (recipe_private = 0 OR recipe_user = " . $SMObj->getUserID() . ") ";
} else {
	$sql .= " recipe_private = 0 ";
}
...
<tr>
        <td align="left" class="title"><?php echo $LangUI->_('Recipe Index');?> - <a href="index.php?m=recipes&amp;a=search"><?php echo $LangUI->_('Search');?></td>
</tr>
</table>

<table border="0" cellpadding="2" cellspacing="1" class="std">
<tr class="indexRow">
        <td><?php echo $LangUI->_('Show');?>:</td>
...
<?php
$sql_count = "SELECT count(*) FROM $db_table_recipes WHERE recipe_name LIKE '".$DB_LINK->addq($where, get_magic_quotes_gpc())."%' AND";
if ($SMObj->isUserLoggedIn()) {
	$sql_count .= " (recipe_private = 0 OR recipe_user = " . $SMObj->getUserID() . ") ";
} else {
	$sql_count .= " recipe_private = 0 ";
}

$sql = "SELECT recipe_id,recipe_name FROM $db_table_recipes WHERE recipe_name LIKE '".$DB_LINK->addq($where, get_magic_quotes_gpc())."%' AND";
if ($SMObj->isUserLoggedIn()) {
	$sql .= " (recipe_private = 0 OR recipe_user = " . $SMObj->getUserID() . ") ";
} else {
	$sql .= " recipe_private = 0 ";
