...
$course_id = (isset($_REQUEST['course_id']) && isValidID($_REQUEST['course_id'])) ?  $_REQUEST['course_id'] : 0;
$base_id = (isset($_REQUEST['base_id']) && isValidID($_REQUEST['base_id'])) ? $_REQUEST['base_id'] : 0;
$ethnic_id = (isset($_REQUEST['ethnic_id']) && isValidID($_REQUEST['ethnic_id'])) ?  $_REQUEST['ethnic_id'] : 0;
$time_id = (isset($_REQUEST['time_id']) && isValidID($_REQUEST['time_id'])) ? $_REQUEST['time_id'] : 0;
$difficult_id = (isset($_REQUEST['difficult_id']) && isValidID($_REQUEST['difficult_id'])) ? $_REQUEST['difficult_id'] : 0;
...
<?php
	...
	if (!isset($_REQUEST["search"])) {
		...
		{
			$query = $query_all . " WHERE recipe_private = 0 " . $query_order;
		}
		else
		{
			$query = $query_all . " WHERE recipe_user = " . $SMObj->getUserID() . $query_order;
		}
	} else {
		#Construct the Query
		...
		if ($keywords != "") {
			...
			$query .= " recipe_comments LIKE '%". $DB_LINK->addq(htmlentities($keywords, ENT_QUOTES), get_magic_quotes_gpc()) . "%') AND";
		}
		if ($SMObj->isUserLoggedIn()) {
			$query .= " (recipe_private = 0 OR recipe_user = " . $SMObj->getUserID() . ") ";
		} else {
			$query .= " recipe_private = 0 ";
		}
		$query .= $query_order;
	}
