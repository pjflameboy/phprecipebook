...
<?php
$sql = "SELECT avg(rating_score) FROM $db_table_ratings WHERE rating_recipe=?";
...
<?php
$sql = "SELECT review_user, review_comments, review_date, user_name FROM $db_table_reviews
		LEFT JOIN $db_table_users ON user_id = review_user WHERE review_recipe=? ORDER BY review_date";
$stmt = $DB_LINK->Prepare($sql);
$rc = $DB_LINK->Execute($stmt, array($recipe_id));
// Error check
DBUtils::checkResult($rc, NULL, NULL, $sql);
