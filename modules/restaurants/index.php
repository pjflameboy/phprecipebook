...
$sql = "SELECT * FROM $db_table_restaurants	
		LEFT JOIN $db_table_prices ON price_id = restaurant_price
		WHERE restaurant_user = '" . $DB_LINK->addq($SMObj->getUserID(), get_magic_quotes_gpc()) ."' ORDER BY restaurant_name";
...
<?php
	while (!$rc->EOF) {
		echo "<tr>\n";
		if (isset($rc->fields['restaurant_website']) && $rc->fields['restaurant_website'] != "") 
		{
		...
		echo '<td><a href="http://www.mapquest.com/maps/map.adp?country=' . 
			(isset($rc->fields['restaurant_country']) ? $rc->fields['restaurant_country'] : '') . 
			...
			(isset($rc->fields['restaurant_country']) ? $rc->fields['restaurant_country'] : '') . '</a></td>';
